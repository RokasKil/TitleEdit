...
namespace CharacterSelectBackgroundPlugin.Windows.Tabs
{
    internal class DisplayTypeTab : ITab
    {
        ...
        private void DrawSelectPresetCombo(string label, CharacterDisplayTypeOption value, Action<CharacterDisplayTypeOption> selectAction, bool showLastLocationOption = true)
        {
            ...
            else if (value.Type == CharacterDisplayType.AetherialSea)
            {
            ...
            GuiUtils.FilterCombo(label, display, () =>
            {
                ...
                if (GuiUtils.FilterSelectable($"Aetherial sea##{Title}##{label}##vanilla aetherial sea", value.Type == CharacterDisplayType.AetherialSea))
                {
                    selectAction.Invoke(new()
                    {
                ...
                DrawDisplayTypeTooltip(CharacterDisplayType.AetherialSea);
        ...
        private void DrawDisplayTypeTooltip(CharacterDisplayType type, string? path = null)
        {
            DrawTooltip(() =>
            {
                switch (type)
                {
                    case CharacterDisplayType.AetherialSea:
